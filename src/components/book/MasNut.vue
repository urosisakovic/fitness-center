<template>
  <div>
    ...
    <div id="success-msg" class="container" role="alert"></div>
...
export default {
  ...
  methods: {
    ...
    validName: function(name) {
      ...
      for (var i = 0; i < name.length; i++)
        if (name.toLowerCase() == name.toUpperCase()) return false;
    ...
    submitButton: function() {
      ...
      if (!this.validEmail(email)) {
        ...
        document.getElementById(
          "email-invalid"
        ).innerHTML = this.words.emailPrivacy;
        document
          .getElementById("email-invalid")
          .style.setProperty("color", "#333333");
      }

      if (!valid) return;

      var downloadPDF = document.getElementById("download-pdf").checked;
      if (downloadPDF) this.savePDF();
    ...
    success: function() {
      ...
      document.getElementById(
        "input-email"
      ).placeholder = this.words.enterEmail;
      document.getElementById(
        "problem-desc"
      ).placeholder = this.words.enterProblemDesc;
  ...
  mounted() {
    ...
    else this.words = require("../../assets/content/sr/dictionary.json").book;
  }
};
</script>